...
def fast_hash(inp: bytes, bit_len=256):
    if bit_len != 256:
...
def slow_hash(inp: bytes, salt: bytes = b'fixed_salt_for_deterministic_hash', bit_len=256):
    byte_len = bit_len//8
    if argon2:
        ...
        return hashlib.pbkdf2_hmac('sha256', inp, salt, 600000, dklen=bit_len//8)
